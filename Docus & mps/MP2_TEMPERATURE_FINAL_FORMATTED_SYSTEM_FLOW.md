# MP2 - Temperature Spring/Bud/Wither - Final Formatted System Flow Simulation

Generated: 2026-06-08 01:48:23

## 1. Program Purpose
This program shows declaration, comment tokenization, spring/bud/wither control flow, comparison operators, and string output.
...
| Line | Code | Meaning |
|---|---|---|
| 1 | root() { | Main function. Interpreter automatically creates a call to root after registration. |
| 2 |     seed temperature = 50; | Variable declaration. Builder creates a declaration AST node; interpreter creates runtime storage. |
| 3 |  | Spacing only. Lexer may produce newline token; parser skips newlines. |
| 4 |     // First check: Is it boiling? | Comment. Lexer creates a comment token; parser skips it; builder removes it before AST. |
| 5 |     spring (temperature >= 50) { | If condition. Interpreter evaluates it first in a spring/bud/wither chain. |
| 6 |         plant("Too hot! The water is boiling!"); | Output statement. Interpreter evaluates arguments and emits text. |
| 7 |     }  | Block brace or structural line. |
| 8 |         bud (temperature <= 0) { | Else-if condition. Interpreter checks it only if spring was false. |
| 9 |         plant("Too cold! The water turned into ice!"); | Output statement. Interpreter evaluates arguments and emits text. |
| 10 |     }  | Block brace or structural line. |
| 11 |     wither { | Else branch. Interpreter runs it only when previous conditions are false. |
| 12 |         plant("The water is just right for watering."); | Output statement. Interpreter evaluates arguments and emits text. |
| 13 |     } | Block brace or structural line. |
| 14 |  | Spacing only. Lexer may produce newline token; parser skips newlines. |
| 15 |     reclaim; | Function return/end statement. Interpreter exits the current function. |
| 16 | } | Block brace or structural line. |

## 6. Lexer Simulation
1. `root` is recognized at scanner.py line 773.
2. `seed temperature = 50;` produces seed at line 812, identifier at line 1292, integer literal at line 1451, plus assignment and semicolon tokens.
3. The comment `// First check: Is it boiling?` becomes a comment token at scanner.py line 2151. It appears in the token table but it is skipped later.
4. `spring`, `bud`, and `wither` are tokenized at scanner.py lines 918, 167, and 1219.
5. `>=` is tokenized at line 2050; `<=` is tokenized at line 1954.
6. Each message in plant is one string literal token from scanner.py line 2726.
...
| Comment skip | Parser skips comment/newline tokens at parser.py line 1207. |
...
1. `parse_variable()` at builder.py line 575 creates the temperature declaration.
2. Before builder runs, parser filters comments/newlines at parser.py line 2252. That is why the comment does not become an AST statement.
3. `parse_if()` at builder.py line 4328 builds one IfStatementNode containing spring, bud, and wither parts.
4. The conditions `temperature >= 50` and `temperature <= 0` become BinaryOpNode expressions.
  ...
  - FunctionDeclaration value=root
    ...
    - Block
      - VariableDeclaration line=2
        ...
        - Identifier value=temperature line=2
        - Value value=50
      - IfStatement line=5
        - Condition line=5
        - Block line=5
        - ElseIfStatement line=5
        - ElseStatement line=5
      - Return line=15
```

## 11. Semantic Simulation
1. Semantic validation checks that `temperature` exists before it is used.
2. It accepts `temperature >= 50` and `temperature <= 0` because comparison expressions return branch/boolean.
3. The plant statements are valid because they output string literals.
4. No semantic errors are found.

## 12. Interpreter Simulation
1. `eval_program()` at interpreter.py line 397 registers root and then calls root.
2. `eval_variable_declaration()` at line 416 stores `temperature = 50`.
3. `eval_if_statement()` at line 2022 evaluates the spring condition first.
4. `temperature >= 50` uses `eval_binary_op()` at line 968 and the `>=` branch at line 1196.
5. Because 50 >= 50 is true, only the spring block runs.
6. `eval_print()` at line 1436 emits `Too hot! The water is boiling!`.
7. The bud and wither blocks are skipped because the spring branch already succeeded.
