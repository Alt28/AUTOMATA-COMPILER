# MP2 - Temperature Spring/Bud/Wither - Very Detailed Step-by-Step Simulation
...
| 2 | `    seed temperature = 50;` | Variable declaration. Builder creates a VariableDeclarationNode; interpreter creates runtime variable storage. |
| 3 | `` | Blank line used only for spacing. |
| 4 | `    // First check: Is it boiling?` | Comment. Lexer records it as a comment token; parser/builder skip it. |
| 5 | `    spring (temperature >= 50) {` | If branch. Interpreter runs this block when condition is true. |
| 6 | `        plant("Too hot! The water is boiling!");` | Output statement. Interpreter evaluates arguments and emits text. |
| 7 | `    } ` | Block closing brace. |
| 8 | `        bud (temperature <= 0) {` | Else-if branch. Interpreter checks it only if spring is false. |
| 9 | `        plant("Too cold! The water turned into ice!");` | Output statement. Interpreter evaluates arguments and emits text. |
| 10 | `    } ` | Block closing brace. |
| 11 | `    wither {` | Else branch. Interpreter runs it only if all previous conditions are false. |
| 12 | `        plant("The water is just right for watering.");` | Output statement. Interpreter evaluates arguments and emits text. |
| 13 | `    }` | Block closing brace. |
| 14 | `` | Blank line used only for spacing. |
| 15 | `    reclaim;` | Return/end statement. Interpreter exits the current function. |
| 16 | `}` | Block closing brace. |
...
1. Parser starts from `<program>` and sees lookahead `root`, so it matches the required root function structure.
2. It accepts local declaration `seed temperature = 50;`.
3. The comment token is skipped by parser.py line 1207. The grammar does not need a comment production.
4. The next meaningful token is `spring`, so parser accepts a spring condition and block.
5. After the spring block, parser sees `bud`, so it accepts the optional else-if branch.
6. After bud, parser sees `wither`, so it accepts the else branch.
7. Finally parser matches required `reclaim;` and closes root.
