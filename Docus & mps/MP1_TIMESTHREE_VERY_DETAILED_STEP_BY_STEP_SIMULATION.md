# MP1 - Function + Cultivate + Spring/Wither - Very Detailed Step-by-Step Simulation
...
| Line | Code | Meaning |
|---|---|---|
| 1 | `pollinate seed timesThree(seed n) {` | Function declaration header before root. |
| 2 | `    reclaim n * 3;` | Return/end statement. Interpreter exits the current function. |
| 3 | `}` | Block closing brace. |
| 4 | `	` | Blank line used only for spacing. |
| 5 | `root() {` | Main function header. Interpreter automatically calls root after registering functions. |
| 6 | `    seed value = 0;` | Variable declaration. Builder creates a VariableDeclarationNode; interpreter creates runtime variable storage. |
| 7 | `	seed i;` | Variable declaration. Builder creates a VariableDeclarationNode; interpreter creates runtime variable storage. |
| 8 | `    cultivate  (i = 1; i <= 4; i++) {` | For-loop style statement: initializer, condition, update, and block. |
| 9 | `        value = timesThree(i);` | Assignment or expression statement. |
| 10 | `        spring (value % 2 == 0) {` | If branch. Interpreter runs this block when condition is true. |
| 11 | `            plant("Even product:",value);` | Output statement. Interpreter evaluates arguments and emits text. |
| 12 | `        } wither {` | Program structure line scanned by lexer and checked by parser. |
| 13 | `            plant("Odd  product:",value);` | Output statement. Interpreter evaluates arguments and emits text. |
| 14 | `        }` | Block closing brace. |
| 15 | `    }` | Block closing brace. |
| 16 | `` | Blank line used only for spacing. |
| 17 | `    reclaim;` | Return/end statement. Interpreter exits the current function. |
| 18 | `}` | Block closing brace. |
...
4. `reclaim n * 3;` becomes reclaim, id, multiplication, integer literal, and semicolon. Multiplication is appended at scanner.py line 1690; integer literals are appended at line 1451.
5. `i <= 4` uses the `<=` token at scanner.py line 1954; `i++` uses increment at line 1897.
6. `value % 2 == 0` uses `%` at scanner.py line 1540 and `==` at line 2004.
...
1. `<program>` starts with lookahead `pollinate`, so the parser accepts a function definition before root.
2. The function header `pollinate seed timesThree(seed n) {` is matched token by token.
3. `reclaim n * 3;` is parsed as a reclaim statement with an expression.
4. After the function closes, parser matches the required `root() {` structure.
5. Inside root, parser accepts local declarations first: `seed value = 0;` and `seed i;`.
6. The parser then accepts `cultivate (i = 1; i <= 4; i++) { ... }` as a for-loop style statement.
7. Inside the loop, `value = timesThree(i);` is parsed as assignment with a function call expression.
8. Then parser checks the spring/wither chain and final `reclaim;`.
