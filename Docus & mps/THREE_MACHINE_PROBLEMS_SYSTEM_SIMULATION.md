...
Generated: 2026-06-08 00:58:50

This revised document explains each MP step by step from editor input until interpreter output. It uses the current system code line numbers.

## A. Same Pipeline Used By All Three MPs
| Step | Explanation |
|---|---|
| 1. User submits code | Frontend sends the whole program text to backend `/api/run`. In `server.py` line 589, the route starts. The JSON body is read at line 152, then the full editor text is stored in `source_code` at line 164. |
| 2. Server calls lexer | At `server.py` line 229, the system calls `lex(source_code)`. This goes to `scanner.py` line 2958. |
| 3. Lexer prepares character scanning | `Lexer.__init__` starts at `scanner.py` line 42. The whole code is stored in `self.source_code` at line 48. `self.current_char` is loaded by `advance()` at line 60. |
| 4. Lexer scans into tokens | `make_tokens()` starts at `scanner.py` line 74. The loop at line 90 keeps reading `current_char`. Every recognized word/symbol/literal is appended as a Token. Tokens and lexical errors return at line 2944. |
| 5. Parser checks syntax | If there are no lexical errors, server calls `parser.parse_and_build(tokens)` at `server.py` line 380. `parse_and_build()` starts at `parser.py` line 2224; `parse()` starts at line 1141. |
| 6. Parser uses stack and CFG | Parser stack starts as `[EOF, <program>]` at `parser.py` line 1162. The loop at line 1192 compares stack top and current token. It selects productions at line 1222, pushes RHS symbols at line 1311, and consumes matching terminals at line 1343. |
| 7. Builder creates AST | After syntax succeeds, parser filters comments/newlines at line 2252, then calls builder at line 2254. `build_ast()` starts at `builder.py` line 41 and resets old symbol table data at line 51. |
| 8. Semantic validates AST | Server calls `validate_ast()` at `server.py` line 402. The wrapper is at `analyzer.py` line 361; validation starts at line 34; `_walk()` recursively visits nodes at line 56. |
| 9. Interpreter executes AST | After semantic success, server creates the interpreter at `server.py` line 663 and calls `interp.interpret(ast)` at line 667. The dispatcher starts at `interpreter.py` line 218. |
...
```
Main server flow: source -> lexer -> parser/builder -> semantic -> interpreter.

### Lexer Initialization And Main Loop
...
```
Shows source_code storage, current_char, advance(), and make_tokens loop.
...
```
Shows token conversion and initial stack.

### Parser Production Selection
...
```
Shows how stack top and lookahead choose a CFG production.

### Builder AST Entry
Lines 41-67 in `Backend\parser\builder.py`
```python
...
```
Shows AST root creation and symbol table reset.
...
74:                 self._walk(child)
```
Shows semantic validation and dynamic AST walking.
...
413: 
```
Shows dispatcher and root() auto-call.
...
### 3. Step-by-Step System Movement
| Step | What Happens |
|---|---|
| Lexer Step 1 | Line 1 begins with `pollinate`. The lexer recognizes it as a reserved word at `scanner.py` line 646. `seed` is recognized at line 812. `timesThree` is not a reserved word, so it becomes `id` at line 1292. |
| Lexer Step 2 | Line 2 `reclaim n * 3;` becomes `reclaim`, `id:n`, `*`, `intlit:3`, `;`. The `reclaim` token is appended at line 741, `*` at line 1690, and integer literal at line 1451. |
| Lexer Step 3 | In root, `cultivate`, `spring`, `wither`, and `plant` are recognized at scanner lines 274, 918, 1219, and 584. Operators `<=`, `++`, `%`, `==` are tokenized at lines 1954, 1897, 1540, and 2004. |
| Parser Step 1 | `<program>` sees lookahead `pollinate`, so the parser accepts a function definition before root. It matches `pollinate seed id ( seed id ) { reclaim expression ; }`. |
| Parser Step 2 | After the function, the parser matches `root ( ) {`. It then checks local declarations first: `seed value = 0;` and `seed i;`. |
| Parser Step 3 | The body statement is a `cultivate` loop. Parser checks the three loop parts: initializer `i = 1`, condition `i <= 4`, update `i++`, then the block. |
| Parser Step 4 | Inside the loop, parser checks `value = timesThree(i);`, then `spring (value % 2 == 0) { plant(...) } wither { plant(...) }`, then final root `reclaim;`. |
| Builder Step 1 | `parse_function()` at `builder.py` line 367 creates AST nodes for `timesThree` and `root`. `parse_return()` at line 4514 builds `reclaim n * 3;`. |
| Builder Step 2 | `parse_variable()` at line 575 builds `value` and `i`. `parse_for()` at line 4590 builds cultivate. `parse_function_call()` at line 3453 checks `timesThree(i)`. `parse_if()` at line 4328 builds spring/wither. |
| Semantic Step | `validate_ast()` walks the AST. It sees the cultivate loop and tracks loop context through `_check_ForLoop` at `analyzer.py` line 208. No semantic errors are found. |
| Interpreter Step 1 | `eval_program()` at `interpreter.py` line 397 registers both function declarations. `main_call = FunctionCallNode("root", [], node.line)` at line 410 starts root. |
| Interpreter Step 2 | Root runs through `eval_function_call()` line 1623. `value` and `i` are created by `eval_variable_declaration()` line 416. The cultivate loop runs in `eval_for_loop()` line 2097. |
| Interpreter Step 3 | Each loop iteration assigns `value = timesThree(i)` using `eval_assignment()` line 658. `timesThree` runs as a function call at line 1623; `n * 3` uses `eval_binary_op()` line 968 and multiplication branch line 1053. |
| Interpreter Step 4 | The condition `value % 2 == 0` is evaluated with modulo branch line 1124 and equality branch line 1150. `eval_if_statement()` line 2022 chooses spring or wither. `plant()` output is handled by `eval_print()` line 1436. |

### 4. Lexer Tokens By Source Line
...
|---|---|---|
| Before loop | value=0, i=0 | root declarations run. |
| Init | i=1 | cultivate initializer assigns i = 1. |
| Iteration 1 condition | 1 <= 4 -> true | Loop body runs. |
| Iteration 1 function | timesThree(1) -> 1 * 3 -> 3 | value becomes 3. |
| Iteration 1 if | 3 % 2 == 0 -> false | wither prints Odd  product: 3. |
| Update 1 | i++ -> i=2 | Next loop check. |
| Iteration 2 | 2 <= 4, timesThree(2)=6, 6%2==0 true | spring prints Even product: 6. |
| Update 2 | i++ -> i=3 | Next loop check. |
| Iteration 3 | 3 <= 4, timesThree(3)=9, 9%2==0 false | wither prints Odd  product: 9. |
| Update 3 | i++ -> i=4 | Next loop check. |
| Iteration 4 | 4 <= 4, timesThree(4)=12, 12%2==0 true | spring prints Even product: 12. |
| Stop | i++ -> i=5; 5 <= 4 -> false | Loop ends, reclaim ends root. |
...
| Step | What Happens |
|---|---|
| Lexer Step 1 | `root` is tokenized at `scanner.py` line 773. `seed temperature = 50;` becomes `seed`, `id:temperature`, `=`, `intlit:50`, `;`; seed is line 812, id is line 1292, intlit is line 1451. |
| Lexer Step 2 | The comment `// First check: Is it boiling?` becomes a `comment` token at scanner line 2151. The parser skips comment/newline tokens at parser line 1207; builder filters them at line 2252. |
| Lexer Step 3 | `spring`, `bud`, and `wither` are reserved words tokenized at scanner lines 918, 167, and 1219. `>=` is line 2050; `<=` is line 1954; strings are line 2726. |
| Parser Step 1 | Parser matches `root ( ) {`, then local declaration `seed temperature = 50;`. |
| Parser Step 2 | The comment token is ignored by the parser. The next meaningful token is `spring`, so parser checks the spring condition, spring block, optional bud block, optional wither block, then root `reclaim;`. |
| Builder Step | `parse_variable()` line 575 builds the temperature declaration. `parse_if()` line 4328 builds one IfStatementNode containing spring, bud, and wither children. |
| Semantic Step | `validate_ast()` line 361 walks the AST. The condition is a comparison expression, so it is valid branch/boolean meaning. No semantic errors are found. |
| Interpreter Step 1 | `eval_program()` line 397 registers root and calls it. `eval_variable_declaration()` line 416 stores `temperature = 50`. |
| Interpreter Step 2 | `eval_if_statement()` line 2022 evaluates `temperature >= 50`. It uses `eval_binary_op()` line 968 and the `>=` branch at line 1196. The result is True. |
| Interpreter Step 3 | Because spring is true, only the spring block runs. `eval_print()` line 1436 emits `Too hot! The water is boiling!`. Bud and wither are skipped. |
...
| Spring condition | temperature >= 50 -> 50 >= 50 -> true | Spring block runs. |
| Output | Too hot! The water is boiling! | plant emits this string. |
| Bud condition | not evaluated | Because spring was already true. |
| Wither block | not executed | Because spring branch already ran. |
| End | reclaim; | root finishes. |
...
| Step | What Happens |
|---|---|
| Lexer Step 1 | `root` is scanner line 773. `seed count;` becomes `seed`, `id:count`, `;`. The identifier path is scanner line 1292. |
| Lexer Step 2 | `cultivate` is tokenized at scanner line 274. The loop condition uses `<=` line 1954; update uses `++` line 1897. |
| Lexer Step 3 | Inside spring, `%` is tokenized at line 1540, `==` at line 2004, and `"Bloom!"` as stringlit at line 2726. |
| Parser Step 1 | Parser matches root, local declaration `seed count;`, then the cultivate statement. |
| Parser Step 2 | For cultivate, parser checks initializer `count = 1`, condition `count <= 5`, update `count++`, then the loop block. |
| Parser Step 3 | Inside the loop block, parser checks spring condition `count % 2 == 0`, spring plant block, wither plant block, then final `reclaim;`. |
| Builder Step | `parse_variable()` line 575 builds count. `parse_for()` line 4590 builds the loop. `parse_if()` line 4328 builds spring/wither. `parse_statement()` line 815 routes each statement type. |
| Semantic Step | `validate_ast()` line 361 walks the loop. `_check_ForLoop()` line 208 enters loop context. No semantic errors are found. |
| Interpreter Step 1 | `eval_variable_declaration()` line 416 creates `count` with default seed value 0. `eval_for_loop()` line 2097 then runs initializer `count = 1`. |
| Interpreter Step 2 | Each condition `count <= 5` uses `eval_binary_op()` line 968 and `<=` branch line 1171. The update `count++` uses `eval_unaryop()` line 1778. |
| Interpreter Step 3 | Each spring condition `count % 2 == 0` uses modulo line 1124 and equality line 1150. `eval_if_statement()` line 2022 chooses `plant("Bloom!")` or `plant(count)`. |
...
| Moment | State / Expression | Result |
|---|---|---|
| Before loop | count = 0 | Default seed value from declaration. |
| Init | count = 1 | cultivate initializer. |
| Iteration 1 | 1 <= 5 true; 1 % 2 == 0 false | wither prints 1; count++ makes 2. |
| Iteration 2 | 2 <= 5 true; 2 % 2 == 0 true | spring prints Bloom!; count++ makes 3. |
| Iteration 3 | 3 <= 5 true; 3 % 2 == 0 false | wither prints 3; count++ makes 4. |
| Iteration 4 | 4 <= 5 true; 4 % 2 == 0 true | spring prints Bloom!; count++ makes 5. |
| Iteration 5 | 5 <= 5 true; 5 % 2 == 0 false | wither prints 5; count++ makes 6. |
| Stop | 6 <= 5 false | Loop ends, reclaim ends root. |
