# MP3 - Count Loop Odd/Even Output - Final Formatted System Flow Simulation
...
| 2 | 	seed count; | Variable declaration. Builder creates a declaration AST node; interpreter creates runtime storage. |
| 3 |     cultivate ( count = 1; count <= 5; count++) { | For-loop style statement: initializer, condition, update, and loop block. |
| 4 |         spring (count % 2  == 0) { | If condition. Interpreter evaluates it first in a spring/bud/wither chain. |
| 5 |             plant("Bloom!"); | Output statement. Interpreter evaluates arguments and emits text. |
| 6 |         } wither { | Block brace or structural line. |
| 7 |             plant(count); | Output statement. Interpreter evaluates arguments and emits text. |
| 8 |         } | Block brace or structural line. |
| 9 |     } | Block brace or structural line. |
| 10 |  | Spacing only. Lexer may produce newline token; parser skips newlines. |
| 11 |     reclaim; | Function return/end statement. Interpreter exits the current function. |
| 12 | } | Block brace or structural line. |
...
6. The spring condition uses `%` at scanner.py line 1552 and `==` at line 2004.
