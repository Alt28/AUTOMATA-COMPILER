# MP1 - Function + Cultivate + Spring/Wither - Final Formatted System Flow Simulation

Generated: 2026-06-08 01:48:23

## 1. Program Purpose
This program shows the complete path for a user-defined function, a cultivate loop, a function call inside assignment, modulo/equality checking, spring/wither branching, and plant output.
...
| Order | Stage | Exact Code Flow | Passes To Next Stage |
|---|---|---|---|
| 0 | Editor/UI sends code | UI run uses Socket.IO `run_code`: server.py lines 752-754. REST `/api/run` is server.py lines 589-591. Both use the same compiler stages. | Full source code string |
| 1 | Read source_code | Socket path stores `source_code = data.get(...)` at server.py line 758. REST path stores `source_code = data['source_code']` at line 602. | `source_code` variable |
| 2 | Lexical analysis | `lex(source_code)` runs at socket line 761 or REST line 605. Lexer setup is scanner.py line 42; `advance()` is line 60; main loop is line 90. | Token list + lexical errors |
| 3 | Syntax + AST build | `parser.parse_and_build(tokens)` runs at socket line 777 or REST line 622. Parser stack starts at parser.py line 1162; builder call happens at line 2254. | AST + builder symbol table |
| 4 | Semantic validation | `validate_ast(ast, symbol_table)` runs at socket line 795 or REST line 641. Analyzer walk starts at analyzer.py line 56. | Validated AST or semantic errors |
| 5 | Interpreter execution | Socket path creates `SessionEmitter` line 842 and `Interpreter` line 844; REST creates collector/interpreter at server.py line 663. Execution calls `interp.interpret(ast)` at socket line 850 or REST line 667. | Runtime output from plant() |
...
## 5. Source Line Meaning
| Line | Code | Meaning |
|---|---|---|
| 1 | pollinate seed timesThree(seed n) { | Function declaration before root. Interpreter registers it first, then runs only when called. |
| 2 |     reclaim n * 3; | Function return/end statement. Interpreter exits the current function. |
| 3 | } | Block brace or structural line. |
| 4 | 	 | Spacing only. Lexer may produce newline token; parser skips newlines. |
| 5 | root() { | Main function. Interpreter automatically creates a call to root after registration. |
| 6 |     seed value = 0; | Variable declaration. Builder creates a declaration AST node; interpreter creates runtime storage. |
| 7 | 	seed i; | Variable declaration. Builder creates a declaration AST node; interpreter creates runtime storage. |
| 8 |     cultivate  (i = 1; i <= 4; i++) { | For-loop style statement: initializer, condition, update, and loop block. |
| 9 |         value = timesThree(i); | Assignment/expression statement that updates runtime variable state. |
| 10 |         spring (value % 2 == 0) { | If condition. Interpreter evaluates it first in a spring/bud/wither chain. |
| 11 |             plant("Even product:",value); | Output statement. Interpreter evaluates arguments and emits text. |
| 12 |         } wither { | Block brace or structural line. |
| 13 |             plant("Odd  product:",value); | Output statement. Interpreter evaluates arguments and emits text. |
| 14 |         } | Block brace or structural line. |
| 15 |     } | Block brace or structural line. |
| 16 |  | Spacing only. Lexer may produce newline token; parser skips newlines. |
| 17 |     reclaim; | Function return/end statement. Interpreter exits the current function. |
| 18 | } | Block brace or structural line. |

## 6. Lexer Simulation
1. The first real word is `pollinate`. The lexer recognizes it as a reserved word and appends a pollinate token at scanner.py line 646.
2. `seed` is recognized as a data type reserved word at scanner.py line 812.
3. `timesThree` is not a reserved word. After the lexer collects its letters, it becomes an identifier token at scanner.py line 1292.
4. `reclaim n * 3;` becomes reclaim, id, multiplication, integer literal, and semicolon. Multiplication is appended at scanner.py line 1702; integer literals are appended at line 1451.
5. `i <= 4` uses the `<=` token at scanner.py line 1954; `i++` uses increment at line 1897.
6. `value % 2 == 0` uses `%` at scanner.py line 1552 and `==` at line 2004.
7. The strings inside plant are collected as whole string literal tokens at scanner.py line 2726. The quote symbols are not separate parser terminals.

## 7. Tokens By Source Line
...
Parser mechanics: stack starts at parser.py line 1162; current stack top is read at line 1196; current lookahead token is read at line 1198; production is selected at line 1222; terminals match at line 1343.

## 9. Builder / AST Simulation
1. `build_ast()` at builder.py line 41 creates the ProgramNode.
2. When builder sees `pollinate`, `parse_function()` at line 367 creates a FunctionDeclarationNode for `timesThree`.
3. `parse_return()` at line 4514 creates the ReturnNode for `reclaim n * 3;`.
4. `parse_variable()` at line 575 creates VariableDeclarationNodes for `value` and `i`.
5. `parse_for()` at line 4590 creates the ForLoopNode.
6. `parse_function_call()` at line 3453 checks that `timesThree(i)` matches the function declaration.
7. `parse_if()` at line 4328 creates the IfStatementNode for spring/wither.

## 10. AST Shape Summary
```text
- Program
  - FunctionDeclaration value=timesThree
    - ReturnType value=seed
    - Parameters
      - Parameter
        - Type value=seed
        - Identifier value=n
  ...
  - FunctionDeclaration value=root
    ...
    - Parameters
    - Block
      - VariableDeclaration line=6
        - Type value=seed line=6
        - Identifier value=value line=6
        - Value value=0
      - VariableDeclaration line=7
        - Type value=seed line=7
        - Identifier value=i line=7
      - ForLoop line=8
        - Assignment line=8
        - Condition line=8
        - Update line=8
...
## 11. Semantic Simulation
1. Semantic validation walks the AST made by builder. It does not scan the source again.
2. `validate_ast()` starts at analyzer.py line 361; `_walk()` at line 56 visits each AST node.
3. The cultivate loop activates `_check_ForLoop()` at analyzer.py line 208, which marks that the validator is inside a loop.
4. No semantic errors are found because `value` and `i` are declared, `timesThree(i)` has the correct argument type, and the spring condition produces branch/boolean.

## 12. Interpreter Simulation
1. `eval_program()` at interpreter.py line 397 registers function declarations first.
2. `eval_function_declaration()` at line 1369 stores `timesThree` and `root` in `self.functions`.
3. After registration, line 410 creates a runtime call to `root()`.
4. `eval_function_call()` at line 1623 runs root in a new scope.
5. `eval_variable_declaration()` at line 416 creates `value = 0` and `i = 0`.
6. `eval_for_loop()` at line 2097 runs the cultivate loop: initializer, condition, body, update, repeat.
7. `value = timesThree(i)` runs through `eval_assignment()` at line 658. The RHS calls `timesThree`, binds parameter `n`, and evaluates `n * 3` using `eval_binary_op()` line 968.
8. The spring condition uses `%` branch at line 1124 and `==` branch at line 1150.
9. `eval_print()` at line 1436 emits each output line.

## 13. Runtime Timeline
| Step | Runtime Moment | What Executes | Result |
|---|---|---|---|
...
## 16. Code Reference Appendix

### Socket.IO run_code Flow
Lines 752-854 in `Backend\server.py`
```python
752: @socketio.on('run_code')
753: # AUTO: Defines function `handle_run_code`.
754: def handle_run_code(data):
755:     # LINE: sid identifies the browser session running this code.
756:     sid = request.sid
757:     # LINE: Read current editor source from Socket.IO payload.
758:     source_code = data.get('source_code', '')
759: 
760:     # LINE: Stage 1, lex source code.
761:     tokens, lex_errors = lex(source_code)
762:     # LINE: Send lexical errors directly to the terminal output.
763:     if lex_errors:
764:         # AUTO: Starts a loop over these values.
765:         for err in lex_errors:
766:             # AUTO: Calls `emit`.
767:             emit('output', {'output': f'Lexical Error: {err}'})
768:         # AUTO: Calls `emit`.
769:         emit('execution_complete', {'success': False, 'stage': 'lexical'})
770:         # AUTO: Returns this result to the caller.
771:         return
772: 
773:     # LINE: Notify frontend that lexical stage passed.
774:     emit('stage_complete', {'stage': 'lexical', 'success': True})
775: 
776:     # LINE: Stage 2, parse and build AST.
777:     parse_result = parser.parse_and_build(tokens)
778:     # LINE: Stop if syntax/builder failed.
779:     if not parse_result['success']:
780:         # AUTO: Sets `error_stage`.
781:         error_stage = parse_result.get('error_stage', 'syntax')
782:         # AUTO: Starts a loop over these values.
783:         for err in parse_result['errors']:
784:             # AUTO: Calls `emit`.
785:             emit('output', {'output': f'{err}'})
786:         # AUTO: Calls `emit`.
787:         emit('execution_complete', {'success': False, 'stage': error_stage})
788:         # AUTO: Returns this result to the caller.
789:         return
790: 
791:     # LINE: Notify frontend that syntax stage passed.
792:     emit('stage_complete', {'stage': 'syntax', 'success': True})
793: 
794:     # LINE: Stage 3, semantic validation.
795:     semantic_result = validate_ast(parse_result['ast'], parse_result['symbol_table'])
796:     # LINE: Stop if semantic validation failed.
797:     if not semantic_result['success']:
798:         # AUTO: Starts a loop over these values.
799:         for err in semantic_result['errors']:
800:             # AUTO: Calls `emit`.
801:             emit('output', {'output': f'{err}'})
802:         # AUTO: Calls `emit`.
803:         emit('execution_complete', {'success': False, 'stage': 'semantic'})
804:         # AUTO: Returns this result to the caller.
805:         return
806: 
807:     # LINE: Notify frontend that semantic stage passed.
808:     emit('stage_complete', {'stage': 'semantic', 'success': True})
809: 
810:     # LINE: Save validated AST before execution.
811:     ast = semantic_result['ast']
812: 
813:     # LINE: Cancel any previous interpreter still waiting/running for this session.
814:     old_interp = interpreters.get(sid)
815:     # AUTO: Checks this condition.
816:     if old_interp and hasattr(old_interp, '_cancelled'):
817:         # AUTO: Sets `old_interp._cancelled`.
818:         old_interp._cancelled = True
819:         # AUTO: Starts a loop over these values.
820:         for evt in list(old_interp.input_events.values()):
821:             # AUTO: Starts protected code that can catch errors.
822:             try:
823:                 # AUTO: Calls `evt.send`.
824:                 evt.send(None)
825:             # AUTO: Handles the matching error case.
826:             except (AttributeError, AssertionError):
827:                 # AUTO: Starts protected code that can catch errors.
828:                 try:
829:                     # AUTO: Calls `evt.set`.
830:                     evt.set()
831:                 # AUTO: Handles the matching error case.
832:                 except Exception:
833:                     # AUTO: Does nothing for this required block.
834:                     pass
835: 
836:     # Runs in the background so water() can pause and receive browser input.
837:     # AUTO: Defines function `run_interpreter`.
838:     def run_interpreter():
839:         # AUTO: Starts protected code that can catch errors.
840:         try:
841:             # LINE: Emitter sends plant()/water() events to this browser session only.
842:             session_emitter = SessionEmitter(socketio, sid)
843:             # LINE: Create one interpreter instance for this run.
844:             interp = Interpreter(socketio=session_emitter)
845:             # LINE: Runtime cancellation flag used when rerunning code.
846:             interp._cancelled = False
847:             # LINE: Store interpreter so capture_input can find it later.
848:             interpreters[sid] = interp
849:             # LINE: Execute the validated AST.
850:             interp.interpret(ast)
851:             # LINE: Tell UI execution completed if run was not cancelled.
852:             if not interp._cancelled:
853:                 # AUTO: Sets `socketio.emit('execution_complete', {'success': True, 'stage': 'execution'}, to`.
854:                 socketio.emit('execution_complete', {'success': True, 'stage': 'execution'}, to=sid)
```
This is the normal browser run path.

### REST /api/run Flow
Lines 589-671 in `Backend\server.py`
```python
...
671:                 'success': True,
```
Same stage order, used by REST run calls.

### Lexer current_char Loop
...
```
Character-by-character scan setup.
...
```
LL(1) stack and lookahead parser.
...
```
Token-to-AST conversion starts here.
...
```
Runtime dispatcher and root call.