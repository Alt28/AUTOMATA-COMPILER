      ...
      term.write('\r\n');
          ...
          const response = await fetch(`${API_BASE}/api/lex`, {
                  ...
                  const displayType = (tok) => {
                    ...
                    const literalTypes = new Set(['intlit','dbllit','strlit','strnglit','chrlit','TT_INTEGERLIT','TT_DOUBLELIT','TT_STRINGLIT','TT_CHARLIT','seedlit','treelit','leaflit']);
                  ...
                  const classifyType = (tok) => {
                    ...
                    if (t === 'strlit' || t === 'strnglit' || t === 'TT_STRINGLIT') return 'string';
                    if (t === 'chrlit' || t === 'TT_CHARLIT') return 'char';
                  ...
                  visibleTokens.forEach(token => {
                      ...
                      const tDisp = token.type || ''; // Token column
                      const cDisp = classifyType(token);  // Type column (RW, ID, integer, etc.)
    ...
    return true;
            ...
            try {
              ...
              if (data.tokens && data.tokens.length > 0) {
                data.tokens.forEach(tok => {
                  ...
                  const cDisp = classifyType(tok); // Type (RW, ID, integer, etc.)
              ...
              } else if (data.stage === 'syntax_error' && data.errors.length > 0) {
                // Syntax errors
                if (!silent) {
                  data.errors.forEach(err => term.write(`${err}\r\n`));
                }
          ...
          window.runSemantic = async function (options = {}) {
            ...
            try {
              ...
              // Display Type function removed (now inline above)
              function displayType(tokenType) {
                const typeMappings = {
                  'id': 'identifier', 'seed': 'keyword', 'tree': 'keyword', 'leaf': 'keyword',
                  'branch': 'keyword', 'vine': 'keyword', 'water': 'keyword', 'plant': 'keyword',
                  'spring': 'keyword', 'wither': 'keyword', 'bud': 'keyword', 'harvest': 'keyword',
                  'grow': 'keyword', 'cultivate': 'keyword', 'tend': 'keyword', 'empty': 'keyword',
                  'prune': 'keyword', 'skip': 'keyword', 'reclaim': 'keyword', 'root': 'keyword',
                  'pollinate': 'keyword', 'variety': 'keyword', 'fertile': 'keyword', 'soil': 'keyword',
                  'bundle': 'keyword', '+': 'operator', '-': 'operator', '*': 'operator', '/': 'operator',
                  '%': 'operator', '**': 'operator', '=': 'operator', '==': 'operator', '+=': 'operator',
                  '-=': 'operator', '*=': 'operator', '/=': 'operator', '%=': 'operator', '`': 'operator',
                  '>': 'operator', '<': 'operator', '>=': 'operator', '<=': 'operator', '!=': 'operator',
                  '&&': 'operator', '||': 'operator', '!': 'operator', '~': 'operator', '++': 'operator',
                  '--': 'operator', '{': 'delimiter', '}': 'delimiter', '[': 'delimiter', ']': 'delimiter',
                  '(': 'delimiter', ')': 'delimiter', ';': 'delimiter', ',': 'delimiter', ':': 'delimiter',
                  'intlit': 'literal', 'dblit': 'literal', 'chrlit': 'literal', 'strlit': 'literal'
                };
                return typeMappings[tokenType] || 'unknown';
              }
              
              // Classify Type function (reuse from lexer)
              function classifyType(displayType) {
                if (displayType === 'keyword') return 'keyword';
                if (displayType === 'operator') return 'operator';
                if (displayType === 'delimiter') return 'delimiter';
                if (displayType === 'literal') return 'literal';
                if (displayType === 'identifier') return 'identifier';
                return 'unknown';
              }
